...
// Fecha o modal de edição do perfil
...
async function salvarPerfil() {
  ...
  try {
    ...
    if (window.simpleSecureClient && window.simpleSecureClient.initialized) {
      const result = await window.simpleSecureClient.saveProfile(dados);
      
      if (result.status === "ok") {
        localStorage.setItem("arroba", arroba);
        alert(result.msg || "Perfil atualizado!");
        atualizarPerfil();
        fecharModal();
      } else {
      ...
      // Fallback: usar método não criptografado
      const response = await fetch("../backend/perfil.php?action=salvar", {
        method: "POST",
...
async function alternarTipoUsuario() {
  ...
  try {
    // Tentar usar criptografia primeiro
    if (window.simpleSecureClient && window.simpleSecureClient.initialized) {
      await window.simpleSecureClient.saveProfile(dadosPerfil);
    } else {
      // Fallback: usar método não criptografado
      await fetch("../backend/perfil.php?action=salvar", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
